...
settings = dict(
    ...
    stop_loss_perc = 0.065,       # % оставания от цены
    stop_loss_fixed = 150000,        # Изначальный stop-loss, можно установить руками нужную сумму, потом бот подтянет.
                                # Можно указать 0, тогда бот высчитает, возьмет текущую цену и применит к ней процент
    amount = 0.00033,             # Кол-во монет, которое планируем продать (в случае Long) или купить (в случае Short)
...
multiplier = 1 #-1 if settings['strategy'] == "Long" else 1
mstart = -1 #1 if settings['strategy'] == "Long" else -1 # startrate множитель
...
while True:
    try:
        ...
        print('chto 3a xyuHR')
        print("tekyshie kypcbi ask {ask:0.8f}, BblbpaHa {cr:0.8f} stop_loss {sl:0.8f}".format(
            ask=ask,  cr=curr_rate, sl=settings['stop_loss_fixed']
        ))
        ...
        if settings['strategy'] == "Short":
            # Bbl6paHa cTpaTeru9l Short, nblTaemc9l kynuTb moHeTbl kak mo)l(Ho BblrogHee
            if curr_rate < settings['stop_loss_fixed']:
                print("ueHa Hu)l(e Stop-Loss")
                if curr_rate_applied < settings['stop_loss_fixed']:
                    print("nopa N3MEHRTb stop-loss, HOBOE 3HA4EHNE {sl:0.8f}".format(sl=curr_rate_applied))                    
                    settings['stop_loss_fixed'] = curr_rate_applied; print('slfixed= ' , settings['stop_loss_fixed'])
            else:
                # ueHa nogH9lJlacb BblWe Stop-Loss, nokynka no pblHky
                b_api.sync()

                tmsp = (int(time.time()) + 946684800 -1) * 1000
                tmsp
                API_KEY='2w22w2w2w2w2wwwwwwwww3ee3e3e33e3ee3e3e3e3e3e3e3e3eA8PeTjx7e4Ewe' 
                API_SECRET = bytearray(b'3e3e3e3e3e3e3e3e3e3e3e3e3e3e3eB7rmPaSUIKJsxLhUTTalrHqpUz')
                symbol='BTCUSDT'
                amount = 0.0003
                side ='BUY'
                type = 'MARKET'
                ...
                from hashlib._sha256 import sha256
                                                   # msg = msg ; msg
                sign = hmac.new(
                                b'x22323232323232323232323723232323232U2323232323UTTalrHqpUz',
                ...
                if 'orderId' in res.text:
                        # Co3gaHue opgepa npowJlo ycnewHo, Bblxog
                    break
